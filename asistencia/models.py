...
class Personas(models.Model):
    ...
    sexo = models.CharField(
        max_length=10, choices=(("masculino", "Masculino"), ("femenino", "Femenino"))
    )
...
class Profesores(models.Model):
    personas = models.ForeignKey(Personas, on_delete=models.CASCADE, unique=True)
    ...
    codigo_qr = models.ImageField(
        ...
        help_text='ESTE CODIGO SE GENERARA AUTOMATICAMENTE y para descargarlo presione el click derecho del mouse sobre el enlace con la ruta, luego "guargar como", y a continuacion seleccione la carpeta de destino (solo si posee los permisos)',
        upload_to="qr/",
    )
    ...
    def save(self, *args, **kwargs):
        ...
        dst = os.path.join(settings.MEDIA_ROOT, "qr", "%s.png" % nombre)
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        shutil.copy(src, dst)
        self.codigo_qr = "qr/%s.png" % nombre
